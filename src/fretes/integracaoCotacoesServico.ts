...
import type { CanalOrigemProposta, ExtracaoProposta, PropostaFrete, RespostaCotacao, SolicitacaoCotacao } from './tipos.js';
...
export async function servicoSolicitarCotacoes(
  ...
  const criadas: SolicitacaoCotacao[] = [];
  ...
  // Nota (Fase 4A.1): o disparo real ao n8n (e-mail/WhatsApp) é escopo da Fase 4A.2 — aqui a
  // solicitação nasce e permanece `PENDENTE_ENVIO`, rastreável na tela da cotação (seção 35).
  return criadas;
}
