...
import type { FechamentoFrete, ModoCalculoFechamento } from './tipos.js';
...
export async function inserirFechamento(cliente: PoolClient, dados: DadosNovoFechamento): Promise<FechamentoFrete> {
  ...
  const { rows } = await cliente.query<LinhaFechamento>(
    `INSERT INTO ${nomeTabelaFechamentos()}
       (id, cotacao_id, proposta_id, transportadora_id, custo_frete, percentual_acrescimo, valor_acrescimo,
        valor_frete_cliente, modo_calculo, usuario_fechamento, observacoes)
